...
def generate_custom_id(prefix: str, field_name: str, table_name: str, conn) -> str:
    ...
    while True:
        ...
        cursor.execute(f"SELECT COUNT(*) FROM {table_name} WHERE {field_name} = ?", (candidate_id,))
...
def save_test_result_auto(data):
    ...
    try:
        ...
        for key in required_fields:
            if not data.get(key):
                raise ValueError(f"Field '{key}' wajib diisi dan tidak boleh kosong.")

        # Generate ID unik (cek di DB)
        id_test_result = generate_custom_id("PRJ.QA.TS", "id_test_result", "test_result", conn)
        id_test_result_history = generate_custom_id("PRJ.QA.TSH", "id_test_result_history", "test_result_history", conn)
